...
class SubServiceGalleryManager {
    ...
    showToast(message, type = 'success') {
        ...
        toast.className = `toast-alert bg-white border ${isSuccess ? 'border-primary/20' : 'border-red-100'} shadow-[0_4px_12px_rgba(0,0,0,0.05)] rounded-md p-3.5 flex items-start gap-3 w-max min-w-[300px] max-w-md pointer-events-auto transform transition-all duration-300 opacity-0 scale-95`;

        const iconBg = isSuccess ? 'bg-primary/10' : 'bg-red-50';
        const iconColor = isSuccess ? 'text-primary' : 'text-red-600';
