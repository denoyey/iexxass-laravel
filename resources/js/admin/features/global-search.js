export function initGlobalSearch() {
    ...
    function performSearch(query) {
        ...
        fetch(`/ix-core/global-search?query=${encodeURIComponent(query)}`, {
            headers: {
    ...
    function renderResults(data, query) {
        ...
        for (const [type, items] of Object.entries(groupedData)) {
            ...
            items.forEach(item => {
                ...
                const highlightedTitle = item.title.replace(regex, '<span class="text-primary font-bold underline decoration-primary decoration-2 underline-offset-2">$1</span>');
