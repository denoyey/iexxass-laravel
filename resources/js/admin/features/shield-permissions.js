    ...
    initTabs() {
        ...
        this.tabs.forEach(tab => {
            tab.addEventListener('click', (e) => {
                ...
                this.tabs.forEach(t => {
                    t.classList.remove('bg-primary/10', 'text-primary', 'border-primary/20', 'active');
                    t.classList.add('bg-white', 'text-gray-600', 'border-transparent');

                    const badge = DomHelper.select('.tab-badge', t);
                    if (badge) {
                        badge.classList.remove('bg-white', 'border-primary/20', 'shadow-sm');
                        badge.classList.add('bg-primary/10');
                    }
                });

                tab.classList.remove('bg-white', 'text-gray-600', 'border-transparent');
                tab.classList.add('bg-primary/10', 'text-primary', 'border-primary/20', 'active');

                const activeBadge = DomHelper.select('.tab-badge', tab);
                if (activeBadge) {
                    activeBadge.classList.remove('bg-primary/10');
                    activeBadge.classList.add('bg-white', 'border-primary/20', 'shadow-sm');
                }
