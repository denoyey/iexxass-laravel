...
@php
    ...
    $pageSubtitle = 'Buat data layanan baru yang akan ditampilkan di halaman depan.';
@endphp
...
@section('main-content')
    ...
    <div class="w-full">
        <div class="bg-white rounded-md shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-6">
                <form action="{{ route('admin.services.items.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Icon Upload (for Card) -->
                        <div class="md:col-span-1">
                            <x-admin.forms.image-upload id="icon" name="icon" label="Icon Layanan (Untuk Card)"
                                :required="false" helpText="Unggah icon. Format: JPG, PNG, WEBP, SVG. Maks 2MB." />
                        </div>

                        <!-- Detail Image Upload -->
                        <div class="md:col-span-1">
                            <x-admin.forms.image-upload id="detail_image" name="detail_image" label="Gambar Detail Layanan"
                                :required="false" helpText="Unggah gambar besar. Format: JPG, PNG, WEBP. Maks 2MB." />
                        </div>

                        <div class="md:col-span-1">
                            <x-admin.forms.form-input name="title[id]" label="Nama Layanan (ID)" :required="true"
                                :value="old('title.id')" placeholder="Contoh: Web Developer" />
                        </div>
                        <div class="md:col-span-1">
                            <x-admin.forms.form-input name="title[en]" label="Nama Layanan (EN)" :required="false"
                                :value="old('title.en')" placeholder="Contoh: Web Developer" />
                        </div>

                        <div class="md:col-span-2">
                            <x-admin.forms.form-input name="price" label="Harga (Opsional)" :required="false"
                                :value="old('price')" placeholder="Contoh: Start From Rp. 3.500.000 / $225" />
                        </div>
                        ...
                        <div class="md:col-span-1">
                            <div class="mb-4">
                                ...
                                <textarea id="description_id" name="description[id]" rows="4" required
                                    ...
                                    placeholder="Deskripsi singkat layanan...">{{ old('description.id') }}</textarea>
                            </div>
                        </div>
                        <div class="md:col-span-1">
                            <div class="mb-4">
                                ...
                                <textarea id="description_en" name="description[en]" rows="4"
                                    class="w-full text-[12px] sm:text-[13px] border border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary p-2"
                                    placeholder="Brief service description...">{{ old('description.en') }}</textarea>
                            </div>
                        </div>
                            ...
                            <div id="features-container" class="space-y-3">
                                <div class="flex items-center gap-2 feature-row bg-white rounded-md">
                                    ...
                                    <div class="flex-1 flex flex-col md:flex-row gap-2">
                                        ...
                                        <input type="text" name="features[en][]"
                                            class="flex-1 text-[12px] sm:text-[13px] border border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary p-2"
                                            placeholder="Fitur (EN)">
                                    </div>
                                    <button type="button"
                                        class="btn-remove-feature px-3 py-2 bg-red-100 text-red-600 rounded-md hover:bg-red-200 transition-colors hidden">
