<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center gap-2 mb-6 permission-tabs">
        <button type="button" data-target="resources"
            ...
            Resources
            <span
                class="tab-badge bg-white text-primary px-1.5 py-0.5 rounded text-[11px] font-bold border border-primary/20 shadow-sm">{{ count($groupedPermissions) }}</span>
        </button>
        @if (count($customPermissions) > 0)
            <button type="button" data-target="custom"
                ...
                Custom Permissions
                <span
                    class="tab-badge bg-primary/10 text-primary px-1.5 py-0.5 rounded text-[11px] font-bold">{{ count($customPermissions) }}</span>
            </button>
        @endif
    </div>
                ...
                <div
                    ...
                    <div class="p-4">
                        <label
                            ...
                            <input type="checkbox"
                                class="select-all-checkbox w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary transition-colors cursor-pointer"
                                aria-label="Select all permissions for {{ $resource }}">
                            <span
                                class="text-[13px] font-semibold text-primary group-hover:text-primary-dark transition-colors">Select
                                all</span>
                        </label>
                            ...
                            @foreach ($data['permissions'] as $perm)
                                <label class="flex items-start gap-2 cursor-pointer group">
                                    <input type="checkbox" name="permissions[]" value="{{ $perm }}"
                                        class="permission-checkbox mt-[2px] w-4 h-4 shrink-0 text-primary border-gray-300 rounded focus:ring-primary transition-colors cursor-pointer"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-4">
                    @foreach ($customPermissions as $perm => $label)
                        <label class="flex items-start gap-2.5 cursor-pointer group">
                            <input type="checkbox" name="permissions[]" value="{{ $perm }}"
                                class="mt-[2px] w-4 h-4 shrink-0 text-primary border-gray-300 rounded focus:ring-primary transition-colors cursor-pointer"
                                {{ in_array($perm, $rolePermissions ?? []) ? 'checked' : '' }}>
                            <span
