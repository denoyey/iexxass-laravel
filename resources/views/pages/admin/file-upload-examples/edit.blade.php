                ...
                <form action="{{ route('admin.file-upload-examples.update', $fileUploadExample) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                        <div class="md:col-span-2">
                            <x-admin.forms.form-input name="judul" label="Judul Konten" :required="true"
                                :value="$fileUploadExample->judul" />
                        </div>

                        <div class="md:col-span-2">
                            <x-admin.forms.form-input name="deskripsi" label="Deskripsi" type="textarea" :rows="3"
                                :required="true" :value="$fileUploadExample->deskripsi" />
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                        <x-admin.forms.image-upload id="cover_image" name="cover_image" label="Cover Image (Single Upload)"
                            :required="false"
                            helpText="Unggah 1 gambar sebagai cover utama. Format: JPG, PNG, WEBP. Maks 2MB."
                    ...
                    <div class="mb-8" id="gallery-section" data-sub-service-id="{{ $fileUploadExample->id_file_upload }}"
                        data-csrf-token="{{ csrf_token() }}" data-base-url="/ix-core/file-upload-examples">
                                    ...
                                    @foreach ($fileUploadExample->images as $img)
                                        <div class="w-[200px] shrink-0 bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm relative flex flex-col"
                                            ...
                                            <div
                                                ...
                                                @if ($img->is_cover)
                                                    <div
                                                        class="absolute top-2 left-2 bg-primary text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">
                                                        COVER
                                                    </div>
                                                @endif
                                            </div>
                                                ...
                                                <div>
                                                    ...
                                                    <div class="flex gap-1">
                                                        <input type="text"
                                                            ...
                                                            class="w-full text-[10px] border border-gray-200 rounded px-1.5 py-1 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors">
                                                        <button type="button"
                                                            class="btn-save-alt bg-primary hover:bg-primary-dark text-white rounded px-1.5 py-1 text-[9px] font-medium transition-colors shadow-sm whitespace-nowrap"
                                                ...
                                                <div
                                                    ...
                                                    <label class="flex items-center gap-1.5 cursor-pointer">
                                                        <input type="radio" name="existing_cover"
                                                            ...
                                                            class="btn-set-cover w-3.5 h-3.5 text-primary focus:ring-primary border-gray-300"
                                                            data-image-id="{{ $img->id_file_upload_image }}">
