...
@section('main-content')
    <div class="mb-3 bg-white rounded-md shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6">
            <form action="{{ route('admin.multi-upload-examples.store') }}" method="POST" enctype="multipart/form-data"
                id="form-multi-upload">
                @csrf
                <x-admin.forms.multi-image-upload id="images" name="images[]" label="Pilih Gambar untuk Galeri"
                    :required="true" :hideCover="true" />

                <div class="pt-4 sm:pt-5 border-t border-gray-200 flex items-center justify-end">
                    <button type="submit"
                        class="px-4 py-1.5 sm:px-5 sm:py-2 text-[11px] sm:text-[13px] font-medium text-white bg-primary hover:bg-primary-dark rounded-md transition-colors shadow-sm">
                        Simpan Gambar
                    </button>
                </div>
            </form>
    ...
    <div class="bg-white rounded-md shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-5 border-b border-gray-100 flex items-center justify-between">
            ...
            @if ($images->count() > 0)
                <form action="{{ route('admin.multi-upload-examples.destroyAll') }}" method="POST"
                    class="form-delete-action m-0" data-message="Semua gambar galeri akan dihapus permanen.">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
            ...
            @if ($images->count() > 0)
                <div class="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory scrollbar-thin">
                    @foreach ($images as $img)
                        <div
                            class="relative rounded-md overflow-hidden border border-gray-200 shadow-sm flex flex-col bg-gray-50 group shrink-0 w-28 sm:w-32 md:w-36 snap-start">
                            <div class="aspect-square w-full relative p-4 flex items-center justify-center bg-white">
                                <img src="{{ asset('storage/' . $img->image_path) }}" width="200" height="200"
                                    class="max-w-full max-h-full object-contain"
                                    alt="{{ $img->alt_text ?? 'Gambar Galeri' }}">
                            </div>
                            ...
                            <div class="flex items-stretch mt-auto border-t border-gray-200 bg-gray-50/50">
                                <button type="button"
                                    ...
                                    data-logo-id="{{ $img->id_multi_upload }}" data-alt-text="{{ $img->alt_text }}"
                                    data-img-src="{{ asset('storage/' . $img->image_path) }}" aria-label="Edit Gambar">
    ...
    <!-- Edit Modal -->
    <div id="editLogoModal" class="fixed inset-0 z-100 hidden"
        data-base-route="{{ route('admin.multi-upload-examples.index') }}">
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            <div class="relative bg-white rounded-md shadow-2xl text-left overflow-hidden transform transition-all sm:my-8 sm:max-w-md w-full scale-95 opacity-0"
                ...
                <form id="editLogoForm" action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="px-6 py-5">
                        <div class="mb-4">
                            <x-admin.forms.image-upload id="edit_image" name="image" label="Ubah Gambar (Opsional)"
                                :required="false" />
                        </div>

                        <div>
                            <label for="edit_alt_text"
                                class="block text-[12px] sm:text-[13px] font-medium text-gray-700 mb-1">Alt Text
                                (SEO)</label>
                            <input type="text" id="edit_alt_text" name="alt_text"
                                class="w-full text-[12px] sm:text-[13px] border border-gray-200 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                    ...
                    <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3">
                        ...
                        <button type="submit"
                            class="px-4 py-2 text-[12px] font-medium text-white bg-primary hover:bg-primary-dark rounded-md transition-colors shadow-sm">
                            Simpan Perubahan
                        </button>
                    </div>
                </form>
