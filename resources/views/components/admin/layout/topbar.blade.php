    ...
    <div class="flex items-center gap-2 sm:gap-4 flex-1">
        <div class="flex items-center shrink-0">
            <button id="sidebar-toggle" aria-label="Toggle Sidebar"
                class="hidden lg:flex items-center justify-center w-8 h-8 rounded-md text-gray-500 bg-gray-100 hover:text-primary transition-colors">
            ...
            <button id="sidebar-mobile-open" aria-label="Open Sidebar"
                class="flex lg:hidden items-center justify-center w-8 h-8 rounded-md text-gray-500 bg-gray-100 hover:text-primary transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    <path
                        d="M5 5H13V19H5V5ZM19 19H15V5H19V19ZM4 3C3.44772 3 3 3.44772 3 4V20C3 20.5523 3.44772 21 4 21H20C20.5523 21 21 20.5523 21 20V4C21 3.44772 20.5523 3 20 3H4ZM11 12L7 8.5V15.5L11 12Z">
                    </path>
        ...
        <button type="button" id="global-search-trigger"
            class="group/search relative z-50 flex items-center bg-gray-100/80 hover:bg-gray-100 border border-transparent focus-within:bg-white focus-within:border-primary/40 focus-within:ring-4 focus-within:ring-primary/10 rounded-md px-2.5 sm:px-3 py-1.5 w-full md:w-64 lg:w-72 transition-all duration-200 outline-none text-left">
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="relative" id="topbar-notification-menu">
            <button id="notification-dropdown-btn"
                class="relative flex items-center justify-center w-8 h-8 rounded-full bg-gray-50 text-gray-500 hover:bg-gray-100 hover:text-gray-700 focus:outline-none transition-colors group">
                <svg class="w-4 h-4 transition-transform group-hover:rotate-12" xmlns="http://www.w3.org/2000/svg"
                ...
                <span
                    class="absolute -top-1.5 -right-1.5 w-[18px] h-[18px] flex items-center justify-center text-[9px] font-bold text-white bg-red-500 border-2 border-white rounded-full">2</span>
            </button>
            ...
            <div id="notification-dropdown-menu"
                ...
                <div class="px-3 py-2 border-b border-gray-100 flex items-center justify-between mb-1">
                    ...
                    <div class="flex items-center gap-1">
                        <button type="button" title="Tandai semua dibaca"
                            class="p-1 text-gray-400 hover:text-primary focus:text-primary bg-gray-100 focus:bg-gray-200 rounded-md focus:outline-none transition-colors">
                ...
                <div class="px-3 py-1.5 border-t border-gray-100 text-center mt-1">
                    <a href="#" class="text-[10px] font-medium text-primary hover:underline">View All
                        Notifications</a>
                </div>
            </div>
        </div>
        ...
        <div class="relative" id="topbar-user-menu">
            <button id="user-dropdown-btn"
                class="relative z-9999 flex items-center justify-center w-8 h-8 rounded-full bg-primary text-white hover:ring-2 hover:ring-gray-200 hover:ring-offset-1 focus:ring-2 focus:ring-gray-200 focus:ring-offset-1 transition-all">
            ...
            <div id="user-dropdown-menu"
                ...
                <div class="flex items-center gap-2.5 px-3 pb-2.5 mb-1.5 border-b border-gray-100">
                    <div
                        class="user-display-initial w-8 h-8 rounded-full bg-primary flex items-center justify-center text-white text-xs font-bold shrink-0">
                        {{ strtoupper(substr(auth()->user()->username ?? 'A', 0, 1)) }}
                    </div>
                    <div class="flex flex-col leading-tight min-w-0 flex-1">
                        <span class="user-display-name text-[11px] sm:text-xs font-semibold text-gray-800 truncate"
                            title="{{ auth()->user()->name ?? (auth()->user()->username ?? 'Admin') }}">{{ auth()->user()->name ?? (auth()->user()->username ?? 'Admin') }}</span>
                        <span
                            class="user-display-email inline-flex items-center justify-center text-[9px] font-medium text-primary border border-primary/30 bg-primary/5 rounded px-1.5 py-0.5 mt-0.5 truncate max-w-full w-fit leading-none h-[18px]"
                            title="{{ auth()->user()->email ?? 'admin@gmail.com' }}">{{ auth()->user()->email ?? 'admin@gmail.com' }}</span>
                    </div>
                </div>
